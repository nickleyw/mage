...
final class DeckImportService {
    ...
    private static final Pattern ARCHIDEKT = Pattern.compile("^https?://(?:www\\.)?archidekt\\.com/decks/(\\d+)(?:[/#?].*)?$", Pattern.CASE_INSENSITIVE);
    private static final Pattern MOXFIELD = Pattern.compile("^https?://(?:www\\.)?moxfield\\.com/decks/([A-Za-z0-9_-]+)(?:[/#?].*)?$", Pattern.CASE_INSENSITIVE);
    private static final Pattern MTGTOP8_ID = Pattern.compile("[?&]d=(\\d+)", Pattern.CASE_INSENSITIVE);

    private final Gson gson;

    DeckImportService(Gson gson) {
        this.gson = gson;
    }

    ImportedDeck importUrl(String sourceUrl) throws IOException {
        if (sourceUrl == null || sourceUrl.trim().isEmpty()) {
            throw new IllegalArgumentException("A public deck URL is required.");
        }
        ...
        if (moxfield.matches()) {
            try {
                ...
                throw new IOException("Moxfield blocked direct export. In Moxfield, choose Export, copy the list, and paste it here.");
            }
        }
        if (normalized.matches("^https?://(?:www\\.)?mtgtop8\\.com/(?:event(?:\\.php)?|mtgo)(?:[?#].*)?$")) {
    ...
    private ImportedDeck importText(String sourceUrl, String source, String defaultName, String endpoint) throws IOException {
        String text = download(endpoint, "text/plain").trim();
        if (text.isEmpty() || text.startsWith("<html") || text.startsWith("<!DOCTYPE")) {
            throw new IOException(source + " did not return an exportable text deck.");
        }
        return new ImportedDeck(defaultName, source, sourceUrl, text);
    }
    ...
    private String download(String endpoint, String accept) throws IOException {
        ...
        connection.setRequestProperty("User-Agent", "XMage-Web-Bridge/1.0 (personal deck import)");
